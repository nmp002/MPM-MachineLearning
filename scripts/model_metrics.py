...
def score_model(model, loader, loss_fn=None, print_results=False, make_plot=False, threshold_type='none'):
    ...
    threshold_type_lower = threshold_type.lower()

    if threshold_type_lower == 'roc':
    ...
    elif threshold_type_lower in ('none', 'both'):
        fig_out = {}
        # ROC
        preds[outs > scores['Optimal Threshold from ROC']] = 1
    ...
    if print_results:
        ...
        for key, item in scores.items():
            if 'Confusion' not in key:
                formatted_item = f'{item:.4f}'
                print(f'|\t{key:<35} {formatted_item:>10}\t|')
        print('_____________________________________________________')
